...
def load_h5(h5_filename, normalize=False, include_label=False):
    ...
    data = f['data'][:].astype(numpy.float32)  # (n, 2048, 3)
    # print(data.shape)
    if normalize:
...
def all_h5(parent, normalize=False, include_label=False,
    ...
    if include_label:
        ...
        for xp, yp in zip(x, y):
            if yp[0] in subclasses:     # yp[0].decode('utf-8')
                if sample is None:
                    xf.append(xp)
                else:
                    xf.append(random.choices(xp, k=sample))
                yf.append(numpy.eye(len(subclasses))[subclasses.index(yp[0])])  # yp[0].decode('utf-8')
