...
class EQCNN_partseg(nn.Module):
    ...
    def forward(self, x):
        ...
        global_feature = x.max(dim=-1, keepdim=True)[0].reshape(batch_size, -1)     # [batch, 1024//3*3]
        ...
        global_feature_processed = x.reshape(batch_size, -1)
        ...
        global_feature = x.max(dim=-1, keepdim=True)[0].reshape(batch_size, -1)

        x = global_feature.reshape(batch_size, -1, 1)
        ...
        global_feature_processed = x.reshape(batch_size, -1)          # (batch_size, emb_dim(2046))
        
        l = torch.ones([batch_size, 16, 1]).cuda()
        # l = l.view(batch_size, -1, 1)
        l = self.conv7(l)

        x = torch.cat((x, l), dim=1)

        x = x.repeat(1, 1, num_points)          # [batch, 1024//3*3, n]

        x123 = x123.reshape(batch_size, -1, num_points)     # [batch, 64//3*3*3, n]

        x = torch.cat((x, x123), dim=1)         # [batch, 64//3*3*3 + 1024//3*3, n]
        ...
        return x.permute(0, 2, 1), global_feature_processed
